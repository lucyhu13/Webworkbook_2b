...
function draw() {
  if (mouseIsPressed) {
    ...
    for (var i = iters; i > 0; --i) {
      snBrush
        .addToPos(
          random(-snBrush.step, snBrush.step),
          random(-snBrush.step, snBrush.step)
        )
        .updateSegmentsPos()
        .draw();
  ...
  this.updateSegmentsPos = function () {
    ...
    for (var itr = 1; itr < this.segments; ++itr) {
      if (p5.Vector.dist(this.posArr[itr], this.posArr[itr - 1]) > this.dist) {
        var tmpVector = p5.Vector.sub(this.posArr[itr - 1], this.posArr[itr])
          .normalize()
          .mult(this.dist);
  ...
  this.draw = function () {
    for (var i = this.segments - 1; i > -1; --i) {
      ...
      if (i > 0) {
        ...
        stroke(
          getImgColor(img, this.posArr[i].x + 5, this.posArr[i].y + 5, 230)
        );
...
function keyTyped() {
  ...
  if (~"qw".indexOf(key.toLowerCase())) {
    snBrush.scale = constrain(snBrush.scale, 0.1, 20);
...
function getImgColor(img, x, y, alpha) {
  ...
  return img.pixels
    ? color(
        img.pixels[targetIdx],
        img.pixels[targetIdx + 1],
        img.pixels[targetIdx + 2],
        alpha || img.pixels[targetIdx + 3]
      )
    : 0;
}
